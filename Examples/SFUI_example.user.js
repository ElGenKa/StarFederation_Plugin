  ...
  if ($("#main-login").length == 0 && $("#divMenu").length > 0) {
    ...
    setTimeout(() => {
      sfui.pushPlugin({
        ...
        wndCondition: ['WndPlanet', 'WndFleet'], // При открытии какого окна/окон выполнится callback
        callback: (wnd) => {
          // Параметр wnd хранит ссылку на окно, заявленное в wndCondition или null, если контекст глобальный
        ...
        callbackCondition: (wnd) => {
          // Условие выполнения функции (необязательное)
          // Значение wnd то же, что и в функции callback
          return true;
        },
        isAllowMobile: true, // Разрешить функцию на мобильных устройствах
        disable: false, // Пометка о неактивности (по умолчанию также false) - sfui.pushPlugin вернёт false  
      ...
      sfui.pushPlugin({
        ...
        callback: (wnd) => { console.log('Я запустился!'); }
      })
